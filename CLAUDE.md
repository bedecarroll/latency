...
- `bun run build` - Compile TypeScript and create production build in `public/` directory
- `bun run dev` - Run TypeScript compiler in watch mode for development
- `bun run serve` - Serve the built application (requires `serve` package)
- `bun run serve:python` - Alternative: serve using Python's built-in HTTP server on port 8000
